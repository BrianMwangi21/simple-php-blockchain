...
class Blockchain(object):
    ...
    def create_genesis_block(self):
        return Block(0, self.get_current_time(), "The Beginning", "0")
    ...
    def get_current_time(self):
        ...
        return "{0.day}/{0.month}/{0.year} {0.hour}:{0.minute}:{0.second}".format(now)    # returning time, unnecessary variable removed
    ...
    def is_chain_valid(self):
        ...
        for j in range(1, len(self.chain)):
            current_block, previous_block = self.chain[j], self.chain[j - 1]
    ...
    def print_chain(self):
        print("\n".join([block.print_block_info() for block in self.chain]))
